...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        run([py(v1),"-c","import toolspec; assert toolspec.__version__=='1.6.2'; print('wheel OK')"])
        v2=td/"editable"; venv.EnvBuilder(with_pip=True).create(v2)
        run([py(v2),"-m","pip","install","-e",".[test]"])
        run([py(v2),"-c","import toolspec; assert toolspec.__version__=='1.6.2'; print('editable OK')"])
